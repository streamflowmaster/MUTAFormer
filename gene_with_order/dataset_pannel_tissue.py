...
class Dataset(tud.Dataset):
    def __init__(self, type='train',
                 ...
                 seg_ratio:list=[0,0,1],
                 config:GPTConfig = GPTConfig(),
                 plex_name = 'plex_mapping.xlsx'):

        gene_dict,ehr_dict,label_dict,vaf_dict,AFP_dict,PIVKAII_dict \
            = load_as_dict_with_protein(path='TissueSample_withProtein.xlsx',
                                        if_vaf_sort=config.if_vaf_sort,if_protein=config.if_protein)
        '''
    ...
    def __getitem__(self, index):
        if self.protein and self.if_qpcr:
            ...
            selected = self.batch_make_pcr_format(selected)
            if len(selected) == 0:
                return torch.tensor([0] * self.max_gene_num),\
                       torch.tensor(self.selected_ehr[index]).float(),\
                       torch.tensor(self.select_label[index]), 0
            return self.map_gene(selected),torch.tensor(self.selected_ehr[index]).float(),\
                   torch.tensor(self.select_label[index]),0
        ...
        return self.map_gene(self.selected_gene[index]), torch.tensor(self.selected_ehr[index]).float(), torch.tensor(
