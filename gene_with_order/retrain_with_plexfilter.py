...
from gene_with_order.dataset_pannel import Dataset
...
class Evaluator:
    ...
    def draw_confusion_matrix(self):
        ...
        sns.heatmap(self.confusion_matrix.T,annot=True,
                    xticklabels=['health','cancer',],
                    yticklabels=['health','cancer',],
...
class process:
    ...
    def train_one_step(self):
        self.model.train()
        loss_sum = []
        for gene,ehr,label in self.train_dataset:
    ...
    def val_one_step(self):
        self.model.eval()
        loss_sum = []
        for gene,ehr,label in self.val_dataset:
            gene = gene.to(self.model.device)
            ehr = ehr.to(self.model.device)
            label = label.to(self.model.device)
    ...
    def test_one_step(self,draw_roc=False,draw_confusion_matrix=False):
        ...
        output_list = []
        label_list = []
        for gene,ehr,label in self.test_dataset:
            gene = gene.to(self.model.device)
            ehr = ehr.to(self.model.device)
            label = label.to(self.model.device)
        ...
        if draw_roc:
            ...
            plt.savefig(f'{self.save_path}/roc_auc_re.svg')
            self.evaluator.pr_curve(output,label)
            plt.savefig(f'{self.save_path}/pr_curve_re.svg')
            plt.close()

        if draw_confusion_matrix:
            self.evaluator.draw_confusion_matrix()
            plt.savefig(f'{self.save_path}/confusion_matrix_re.svg')
            plt.close()
    ...
    def save_log(self):
        ...
        with open(f'{self.save_path}/log.txt','w') as f:
            f.write(log)
    def save_model(self):
        torch.save(self.model.state_dict(),f'{self.save_path}/model_re.pth')

    def load_model(self):
        self.model.load_state_dict(torch.load(f'{self.save_path}/model_re.pth'))
...
if __name__ == '__main__':
    layer = 10
    seed = 11
    config = dict(block_size=10, vocab_size=500, n_layer=layer, n_head=16, n_embd=64, dropout=0.25, bias=True, cls_num=2,
                    apply_ehr = False,if_vaf_sort=True,if_qpcr=True)
    path = 'log_revised_abalation/layer/'+str(layer)+'/'+str(seed)
    config = GPTConfig(**config)
    p = process(config,seg_seed=1,lr=1e-4,batch_size=256,save_path=path)
    p.train(20)
    print(p.log)
    p.save_everything()
