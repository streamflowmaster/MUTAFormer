...
class Dataset(tud.Dataset):
    ...
    def make_pcr_format(self,gene):
        chrom_pos = gene.split(':')[0]
        gene_pos = int(gene.split(':')[1])
        if chrom_pos in self.PCR_chrom:
            for pcr in self.PCR_chrom[chrom_pos]:
                if gene_pos >= self.PCR_bank[pcr][1] and gene_pos <= self.PCR_bank[pcr][2]:
                    return self.pcr_dict[pcr]
        return 'None'
