...
if __name__ == '__main__':
    config = dict(block_size=15, vocab_size=250, n_layer=4, n_head=16, n_embd=64, dropout=0.15, bias=True, cls_num=2,
                    apply_ehr = False,if_vaf_sort=True,if_qpcr=False)
    config = GPTConfig(**config)
    p = process(config,seg_seed=1,lr=1e-3,batch_size=256)
    p.train(20)
    print(p.log)
    p.save_everything()
